"""Code-file parsers shared by the distfind command-line interface.

Supported inputs:
  * Pauli stabiliser strings (one stabiliser per line; I/./_ = identity, X, Y, Z).
    A blank line terminates, mirroring the original distfind. Only CSS codes are accepted, so each
  ...
  * Built-in code generators from ``distfind.codes`` selected by ``NAME[:L]``.
"""
...
def parse_pauli(text: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for raw in text.splitlines():
        line = raw.rstrip("\r")
        if line.strip() == "":
            break  # blank line terminates input, like the original distfind
        lines.append(line)
    if not lines:
...
def parse_stabilizers(text: str) -> np.ndarray:
    ...
    non-CSS codes). Reads lines until a blank line or EOF (mirroring the original distfind). Returns an
