...
3. **Symplectic stabilizer matrix** — pass `--symplectic` pointing at a 0/1 matrix with
   ...
   X-support in the last `n`), for a general non-CSS code.
    ...
    qminweight --symplectic G.txt --subsystem  # non-CSS dressed (gauge) distance
    ```
...
stdin or a file; the development C++ CLI uses qubitserf's convention instead, where the
...
self-orthogonal (where qubitserf returns a nonzero weight — see
