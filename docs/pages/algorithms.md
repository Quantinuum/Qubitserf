...
`distfind` implements **three** deterministic, exact distance algorithms. They compute the
...
- **hybrid CPU/GPU dispatch**: each weight level goes to the GPU only when its candidate
  ...
  so the GPU is never a slowdown (tunable via `DISTFIND_GPU_MIN_WORK`);
...
the enumeration to certify the lower bound is unavoidably deep, so distfind reports a
...
*et al.* recommend (after Pryadko) for sparse CSS codes. distfind's implementation is
...
non-parallelizable, and distfind's own research confirmed it. Its natural parallelism is
*across independent seed qubits*, which distfind exploits on the multicore CPU; that
...
### Correctness (the fix over the original distfind)

This package carries over this feature and its CLI shape from the **original distfind**, but
...
- **BZ — yes** (the default non-CSS path), via the **weight-doubling isometry**
  ...
  ([arXiv:2408.10743](https://arxiv.org/abs/2408.10743)); `distfind` builds it in
...
- **MITM — yes**. The coordinate-split / syndrome-match structure is unchanged; only the
  ...
  exactly the original distfind's "middle algorithm".
...
- L. Hernando, E. S. Quintana-Ortí & M. Grassl, *Improved Brouwer–Zimmermann*,
  [arXiv:2408.10743](https://arxiv.org/abs/2408.10743). The improved BZ that distfind's
