...
Methods: **qubitserf cc** (connected cluster), **qubitserf bz** on cpu and gpu (Brouwer-Zimmermann, capped on hard sparse codes), **qubitserf mitm** (meet-in-the-middle, small n only), and the reference `codeDistance` package's **BZDistMW** and **connectedClusterMW** (subprocess, hard timeout).
...
- Per-code qubitserf budget: 20s; bz only for n <= 144; mitm only for n <= 62; reference subprocess timeout 30s; BZ max_weight cap on hard codes: 6.
- **ref BZDistMW / qubitserf cc speedup**: median 12.8x, max 18232.0x, min 3.0x (over 17 codes).
- **ref connectedClusterMW / qubitserf cc speedup**: median 11.3x, max 821.4x, min 1.1x (over 24 codes).
- **qubitserf bz cpu / gpu speedup**: median 1.1x, max 18.2x, min 0.7x (over 19 codes).
- Fastest-finished method per code: qubitserf cc: 20, qubitserf bz (gpu): 8, qubitserf mitm: 1.
