...
Because the cases span different code families, raw n is not a comparable hardness
axis; we plot runtime against log10(n^d) = d*log10(n), a family-agnostic proxy for
the naive search-space exponent.

    PYTHONPATH=python /opt/miniconda3/envs/sage_env/bin/python bench/bz_vs_cc.py

Writes bench/bz_vs_cc.md and bench/bz_vs_cc.png. Env: TIMEOUT (s, default 330),
...
import qubitserf.codes as codes  # noqa: E402  (PYTHONPATH=python)
...
def _write_md(rows, methods):
    ...
    with open(md, "w") as f:
        ...
        f.write(f"Uniform per-run timeout {TIMEOUT:.0f}s. `d log10(n)` = log10 of the "
                "naive search-space size n^d, a family-agnostic hardness proxy.\n\n")
        f.write("| code | family | n | d | d*log10(n) |")
        for _, mlabel, _, _ in methods:
            f.write(f" {mlabel} |")
        f.write("\n|---|---|---|---|---|" + "---|" * len(methods) + "\n")
        for label, family, n, d, res in rows:
            x = f"{d * math.log10(n):.1f}" if d is not None else "?"
            f.write(f"| {label} | {family} | {n} | {d if d is not None else '?'} | {x} |")
...
def _plot(rows, methods):
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    COLOR = {"cc": "#eda100", "bz_cpu": "#1baf7a", "bz_gpu": "#2a78d6"}
    MARK = {"dense": "o", "sparse": "^"}

    fig, ax = plt.subplots(figsize=(8.4, 5.2), dpi=160)
    for key, mlabel, _, _ in methods:
        for family in ("dense", "sparse"):
            xs, ys, tos = [], [], []
            for label, fam, n, d, res in rows:
                if fam != family or d is None:
                    continue
                dv, dt, to = res[key]
                xs.append(d * math.log10(n))
                ys.append(max(dt, 1e-3))
                tos.append(to)
            solid = [(x, y) for x, y, t in zip(xs, ys, tos) if not t]
            hollow = [(x, y) for x, y, t in zip(xs, ys, tos) if t]
            if solid:
                ax.scatter(*zip(*solid), s=46, marker=MARK[family],
                           color=COLOR[key], zorder=3,
                           label=mlabel if family == "dense" else None)
            if hollow:  # timed out: hollow marker pinned at the timeout line
                ax.scatter(*zip(*hollow), s=46, marker=MARK[family],
                           facecolors="none", edgecolors=COLOR[key],
                           linewidths=1.6, zorder=3,
                           label=mlabel if family == "dense" and not solid else None)

    ax.axhline(TIMEOUT, color="#b0b0ab", lw=1, ls=":", zorder=1)
    ax.text(0.02, TIMEOUT * 1.25, f"timeout ({TIMEOUT:.0f}s; hollow = killed)",
            transform=ax.get_yaxis_transform(), ha="left", fontsize=8, color="#6e6e69")
    # Selective direct labels: only the story-carrying points; the small-code cluster
    # (everything certified in <0.5s by every method) gets one collective note instead.
    LABELLED = {"qbch [[127,71,9]]", "gross [[144,12,12]]",
                "toric L=10 [[200,2,10]]", "qbch [[63,27,7]]"}
    for label, fam, n, d, res in rows:
        if d is None or label not in LABELLED:
            continue
        ymax = max(min(dt, TIMEOUT) for _, dt, _ in res.values())
        ax.annotate(label, (d * math.log10(n), ymax), textcoords="offset points",
                    xytext=(-2, 8), ha="right", fontsize=7, color="#6e6e69")
    ax.text(0.02, 0.30, "unlabeled cluster: small codes (n <= 72)\n"
            "every method certifies in < 0.5 s",
            transform=ax.transAxes, fontsize=8, color="#6e6e69")
    ax.margins(x=0.05)

    ax.set_yscale("log")
    ax.set_ylim(2e-3, TIMEOUT * 8)
    ax.set_xlabel(r"$d\,\log_{10} n$   (log10 of naive search space $n^d$)")
    ax.set_ylabel("wall time (s)")
    ax.set_title("Distance certification: BZ wins on dense checks (circles = BCH),\n"
                 "CC wins on sparse LDPC (triangles)", fontsize=11)
    ax.grid(True, which="major", color="#e8e8e4", lw=0.7, zorder=0)
    ax.set_axisbelow(True)
    for s in ("top", "right"):
        ax.spines[s].set_visible(False)
    ax.legend(frameon=False, fontsize=9, loc="center left")
    fig.tight_layout()
    png = os.path.join(HERE, "bz_vs_cc.png")
    fig.savefig(png)
    print(f"wrote {png}")
