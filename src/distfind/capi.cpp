...
namespace {
void fill(DistfindResult* out, const BZResult& r) {
...
int distfind_stabilizer_operator_weight(
    ...
    if (in_rowspace(mG, op)) {
        ...
        if (m == "mitm") {
            ...
            std::fprintf(stderr,
                "distfind: connected-cluster has no operator-weight form; using the "
...
int distfind_backend_available(const char* name) {
...
const char* distfind_version(void) { return "0.1.0"; }
